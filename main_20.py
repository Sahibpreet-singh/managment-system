...
_DEFAULT_CONFIG = {
    ...
    "whatsapp_msg": (
        ...
        "Rs {amount} ki date {date} ko thi jo abhi tak pending hai.\n\n"
        "Kripya jald se jald bhugtan karein.\n\n"
        "Shukriya\n- {company}"
    ),
    "sms_msg": (
        "Namaskar {name} ji, "
        "Sandhu Enterprises file {file_no} ka due Rs {amount} "
    ...
    "send_whatsapp":  True,
    "send_sms":       True,
    # CallMeBot API key  —  FREE, sends WhatsApp directly via HTTP (no browser)
    # One-time setup: Save +34 644 44 27 06 on your phone → send it a WhatsApp:
    #   "I allow callmebot to send me messages"
    # You will receive your API key via WhatsApp within seconds.
    # NOTE: Each recipient must do this once to receive messages from CallMeBot.
    "callmebot_key":  "",
    # Fast2SMS API key  —  free 500 SMS/day, India only (fast2sms.com)
    "fast2sms_key":   "",
    # Delay in seconds between WhatsApp sends (CallMeBot rate limit ~1/sec)
    "whatsapp_delay": 2,
}
...
def _send_whatsapp_callmebot(phone_e164, message, api_key):
    """
    Send WhatsApp instantly via CallMeBot free API — pure HTTP, no browser.
    phone_e164  e.g. '+919815565672'
    api_key     from CallMeBot (see setup instructions in config)

    CallMeBot limitation: each RECIPIENT must have registered their number
    with CallMeBot once (send "I allow callmebot to send me messages" to
    +34 644 44 27 06 on WhatsApp).
    """
...
def _send_sms_fast2sms(phone_10digit, message, api_key):
    """
    Send SMS via Fast2SMS free API — India only, 500 free SMS/day.
...
def _run_notifications(parent, recipients, cfg, log_widget, done_label):
    """
    Sends WhatsApp (CallMeBot) + SMS (Fast2SMS) directly via HTTP.
    No browser. No human interaction. Runs in a background thread.
    """
    ...
    wa_key      = cfg.get("callmebot_key", "").strip()
    sms_key     = cfg.get("fast2sms_key",  "").strip()
    wa_delay    = max(1, int(cfg.get("whatsapp_delay", 2)))

    sent_wa = 0; failed_wa = 0
    sent_sms = 0; failed_sms = 0
    ...
    if do_wa and not wa_key:
        log("⚠  CallMeBot API key not set — WhatsApp will be skipped.", "warn")
        log("   Enter your key in Settings and save.", "warn")
        do_wa = False

    if do_sms and not sms_key:
        log("⚠  Fast2SMS API key not set — SMS will be skipped.", "warn")
        log("   Enter your key in Settings and save.", "warn")
        do_sms = False

    total = len(recipients)
    log(f"Starting — {total} recipient(s)  |  "
        f"WhatsApp: {'ON' if do_wa else 'OFF'}  |  "
        f"SMS: {'ON' if do_sms else 'OFF'}", "info")
    log("━" * 52, "dim")
    ...
    for idx, r in enumerate(recipients, 1):
        ...
        log(f"\n[{idx}/{total}]  {name}  |  {phone}", "info")

        if not phone_e164:
            log(f"  ⚠  Invalid phone '{phone}' — skipped", "warn")
            continue

        phone_10 = phone_e164.lstrip('+')[2:]   # strip +91 → 10 digits
        ...
        if do_wa:
            ok, result = _send_whatsapp_callmebot(phone_e164, wa_msg, wa_key)
            if ok:
                log(f"  ✅  WhatsApp sent  →  {result[:60]}", "ok")
                sent_wa += 1
            else:
                log(f"  ❌  WhatsApp failed  →  {result[:80]}", "err")
                failed_wa += 1
            # Respect CallMeBot rate limit
            if idx < total:
                _time.sleep(wa_delay)

        # ── SMS via Fast2SMS ────────────────────────────────────────────────
        if do_sms:
            ok, result = _send_sms_fast2sms(phone_10, sms_msg, sms_key)
            if ok:
                log(f"  ✅  SMS sent  →  {result[:60]}", "ok")
                sent_sms += 1
            else:
                log(f"  ❌  SMS failed  →  {result[:80]}", "err")
                failed_sms += 1

    log("\n" + "━" * 52, "dim")
    log(f"Done  |  WhatsApp: {sent_wa} sent, {failed_wa} failed"
        f"  |  SMS: {sent_sms} sent, {failed_sms} failed", "ok")
    try:
        done_label.config(
            text=f"✅  Done — WA: {sent_wa}✓ {failed_wa}✗  |  SMS: {sent_sms}✓ {failed_sms}✗",
            fg=ACCENT2 if (failed_wa + failed_sms == 0) else ACCENT_YEL)
    except Exception:
        pass
...
def notification_window(parent, recipients=None):
    ...
    cols.pack(fill=tk.BOTH, expand=True, padx=16, pady=10)
    ...
    def _populate_recipients():
        ...
        for i, r in enumerate(recipients):
            ...
            rec_tree.insert("", "end", iid=str(i), tags=tag, values=(
                "☑" if phone_ok else "☐",
                r.get("name", ""),
                r.get("phone", ""),
    ...
    def _refresh_sel_marks():
        for i in range(len(recipients)):
            try:
                ...
                if vals:
                    vals[0] = "☑" if _selected.get(i, True) else "☐"
    ...
    def _select_all():
        for i in range(len(recipients)):
            _selected[i] = bool(_clean_phone(recipients[i].get("phone", "")))
        _refresh_sel_marks()
    ...
    tk.Label(btn_row_l, text="Click row to toggle",
             font=(FONT_UI, 10), fg=TEXT_DIM, bg=BG_CARD).pack(side=tk.RIGHT, padx=8)

    _populate_recipients()

    # ════════════════════════════════════════════════════════════════════
    # RIGHT — Settings + Send button (always visible) + Log
    # ════════════════════════════════════════════════════════════════════
    right_col = tk.Frame(cols, bg=BG)
    right_col.grid(row=0, column=1, sticky="nsew")
    right_col.grid_rowconfigure(0, weight=1)   # scrollable settings expands
    right_col.grid_columnconfigure(0, weight=1)

    # ── Scrollable settings area ─────────────────────────────────────────
    settings_canvas = tk.Canvas(right_col, bg=BG, highlightthickness=0)
    settings_sb = tk.Scrollbar(right_col, orient="vertical",
    ...
    settings_win_id = settings_canvas.create_window(
        (0, 0), window=settings_inner, anchor="nw")

    def _resize_settings(e):
        settings_canvas.itemconfig(settings_win_id, width=e.width)
    settings_canvas.bind("<Configure>", _resize_settings)
    settings_inner.bind("<Configure>",
        lambda e: settings_canvas.configure(
            scrollregion=settings_canvas.bbox("all")))
    settings_canvas.bind_all("<MouseWheel>",
        lambda e: settings_canvas.yview_scroll(int(-1*(e.delta/120)), "units"))

    # ── Channel toggles ─────────────────────────────────────────────────
    ch_card = tk.Frame(settings_inner, bg=BG_CARD,
                       highlightbackground=BORDER, highlightthickness=1,
                       padx=14, pady=10)
    ch_card.pack(fill=tk.X, pady=(0, 6))

    tk.Label(ch_card, text="📡  Channels", font=(FONT_UI, 13, "bold"),
             fg=TEXT, bg=BG_CARD).pack(anchor="w")
    tk.Frame(ch_card, bg=BORDER, height=1).pack(fill=tk.X, pady=(4, 6))
    ...
    tk.Checkbutton(ch_row, text="📲  WhatsApp (CallMeBot)",
                   variable=wa_var, font=(FONT_UI, 12),
                   fg=TEXT, bg=BG_CARD, selectcolor=BG_INPUT,
                   activebackground=BG_CARD).pack(side=tk.LEFT)
    tk.Checkbutton(ch_row, text="📩  SMS (Fast2SMS)",
                   variable=sms_var, font=(FONT_UI, 12),
                   fg=TEXT, bg=BG_CARD, selectcolor=BG_INPUT,
                   activebackground=BG_CARD).pack(side=tk.LEFT, padx=12)

    # CallMeBot key
    tk.Label(ch_card, text="CallMeBot API Key:",
             font=(FONT_UI, 11, "bold"), fg=TEXT, bg=BG_CARD).pack(anchor="w", pady=(8, 0))
    tk.Label(ch_card, text="Save +34 644 44 27 06 → WhatsApp: 'I allow callmebot to send me messages'",
             font=(FONT_UI, 9), fg=TEXT_DIM, bg=BG_CARD).pack(anchor="w")
    callmebot_entry = make_entry(ch_card, width=36)
    callmebot_entry.insert(0, cfg.get("callmebot_key", ""))
    callmebot_entry.pack(fill=tk.X, ipady=5, pady=(3, 6))

    # Fast2SMS key
    tk.Label(ch_card, text="Fast2SMS API Key:",
             font=(FONT_UI, 11, "bold"), fg=TEXT, bg=BG_CARD).pack(anchor="w")
    tk.Label(ch_card, text="fast2sms.com → Developer → API  (500 free SMS/day)",
             font=(FONT_UI, 9), fg=TEXT_DIM, bg=BG_CARD).pack(anchor="w")
    api_entry = make_entry(ch_card, width=36)
    api_entry.insert(0, cfg.get("fast2sms_key", ""))
    api_entry.pack(fill=tk.X, ipady=5, pady=(3, 0))

    # Company + delay on one row
    cd_row = tk.Frame(ch_card, bg=BG_CARD)
    cd_row.pack(fill=tk.X, pady=(8, 0))
    cd_row.grid_columnconfigure(1, weight=1)
    tk.Label(cd_row, text="Company:", font=(FONT_UI, 11), fg=TEXT_DIM,
             bg=BG_CARD, anchor="w").grid(row=0, column=0, sticky="w", padx=(0, 4))
    co_e = make_entry(cd_row, width=18)
    co_e.insert(0, cfg.get("company", "Sandhu Enterprises"))
    co_e.grid(row=0, column=1, sticky="ew", ipady=5, padx=(0, 10))
    tk.Label(cd_row, text="Delay(s):", font=(FONT_UI, 11), fg=TEXT_DIM,
             bg=BG_CARD, anchor="w").grid(row=0, column=2, sticky="w", padx=(0, 4))
    dl_e = make_entry(cd_row, width=4)
    dl_e.insert(0, str(cfg.get("whatsapp_delay", 2)))
    dl_e.grid(row=0, column=3, sticky="ew", ipady=5)

    # ── Message templates ────────────────────────────────────────────────
    msg_card = tk.Frame(settings_inner, bg=BG_CARD,
                        highlightbackground=BORDER, highlightthickness=1,
                        padx=14, pady=10)
    msg_card.pack(fill=tk.X, pady=(0, 6))
    ...
    tk.Label(msg_card,
             ...
             font=(FONT_UI, 9), fg=TEXT_DIM, bg=BG_CARD).pack(anchor="w", pady=(2, 4))
    tk.Frame(msg_card, bg=BORDER, height=1).pack(fill=tk.X, pady=(0, 6))

    tk.Label(msg_card, text="WhatsApp:", font=(FONT_UI, 11, "bold"),
             fg=TEXT, bg=BG_CARD).pack(anchor="w")
    ...
    wa_txt.pack(fill=tk.X, pady=(3, 8))

    tk.Label(msg_card, text="SMS (keep short):", font=(FONT_UI, 11, "bold"),
             fg=TEXT, bg=BG_CARD).pack(anchor="w")
    sms_txt = tk.Text(msg_card, font=(FONT_UI, 11), fg=TEXT, bg=BG_INPUT,
                      relief="flat", highlightthickness=1,
                      highlightbackground=BORDER, highlightcolor=ACCENT,
                      height=3, padx=8, pady=6, wrap="word")
    sms_txt.insert("1.0", cfg.get("sms_msg", _DEFAULT_CONFIG["sms_msg"]))
    sms_txt.pack(fill=tk.X, pady=(3, 0))
    ...
    # ── Log (compact, below settings) ───────────────────────────────────
    log_card = tk.Frame(settings_inner, bg=BG_CARD,
                        highlightbackground=BORDER, highlightthickness=1,
                        padx=14, pady=10)
    log_card.pack(fill=tk.X, pady=(0, 6))
    log_card.grid_rowconfigure(1, weight=1)
    log_card.grid_columnconfigure(0, weight=1)

    tk.Label(log_card, text="📜  Send Log", font=(FONT_UI, 12, "bold"),
             fg=TEXT, bg=BG_CARD).pack(anchor="w")

    log_txt = tk.Text(log_card, font=(FONT_MONO, 10), fg=TEXT, bg="#0f172a",
                      relief="flat", highlightthickness=0,
                      padx=8, pady=6, state="disabled", wrap="word", height=8)
    ...
    log_sb2 = tk.Scrollbar(log_card, orient="vertical", command=log_txt.yview)
    log_txt.configure(yscrollcommand=log_sb2.set)

    log_frame = tk.Frame(log_card, bg=BG_CARD)
    log_frame.pack(fill=tk.X, pady=(4, 0))
    log_txt.pack(in_=log_frame, side=tk.LEFT, fill=tk.BOTH, expand=True)
    log_sb2.pack(in_=log_frame, side=tk.RIGHT, fill=tk.Y)

    done_lbl = tk.Label(log_card, text="", font=(FONT_UI, 11, "bold"),
                        fg=ACCENT2, bg=BG_CARD)
    done_lbl.pack(anchor="w", pady=(4, 0))

    # ── SEND BUTTON — fixed at very bottom of right col ─────────────────
    send_frame = tk.Frame(right_col, bg=BG, pady=6)
    send_frame.grid(row=1, column=0, columnspan=2, sticky="ew")

    def _save_cfg():
        cfg["send_whatsapp"]   = wa_var.get()
        cfg["send_sms"]        = sms_var.get()
        cfg["callmebot_key"]   = callmebot_entry.get().strip()
        ...
        try:
            cfg["whatsapp_delay"] = max(1, int(dl_e.get().strip() or 2))
        except Exception:
            cfg["whatsapp_delay"] = 2
    ...
    send_btn = tk.Button(send_frame, text="🚀  Send Notifications  [ F10 ]",
                         command=_do_send,
    ...
    win.bind("<Escape>", lambda e: win.destroy())
    win.bind("<F10>",    lambda e: _do_send())
